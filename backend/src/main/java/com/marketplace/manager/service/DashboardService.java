...
public class DashboardService {
    ...
    public DashboardResponse getDashboardData() {
        ...
        BigDecimal margemBrutaMedia = totalVendido.compareTo(BigDecimal.ZERO) > 0 
            ? lucroBrutoTotal.divide(totalVendido, 4, RoundingMode.HALF_UP).multiply(BigDecimal.valueOf(100)).setScale(2, RoundingMode.HALF_UP) 
            : BigDecimal.ZERO;

        BigDecimal margemLiquidaMedia = totalVendido.compareTo(BigDecimal.ZERO) > 0 
            ? lucroLiquidoTotal.divide(totalVendido, 4, RoundingMode.HALF_UP).multiply(BigDecimal.valueOf(100)).setScale(2, RoundingMode.HALF_UP) 
            : BigDecimal.ZERO;
        ...
        Long estoqueBaixo = products.stream().filter(p -> p.getQuantidadeEstoque() > 0 && p.getQuantidadeEstoque() <= 5).count();
        ...
        DashboardResponse.IndicatorsDTO indicators = DashboardResponse.IndicatorsDTO.builder()
            ...
            .lucroLiquidoTotal(lucroLiquidoTotal)
        ...
        Map<Product, Long> productQtyMap = orders.stream()
            .collect(Collectors.groupingBy(Order::getProduct, Collectors.summingLong(Order::getQuantidade)));

        Map<Product, BigDecimal> productRevenueMap = orders.stream()
            .collect(Collectors.groupingBy(Order::getProduct, Collectors.reducing(BigDecimal.ZERO, Order::getValorVenda, BigDecimal::add)));
        ...
        List<DashboardResponse.TopProductDTO> topProducts = productQtyMap.entrySet().stream()
            .map(entry -> {
                ...
                return new DashboardResponse.TopProductDTO(p.getNome(), entry.getValue(), totalVal);
            })
        ...
        // 3. Category Margins
        Map<Category, List<Order>> categoryOrdersMap = orders.stream().collect(Collectors.groupingBy(Order::getCategory));
        List<DashboardResponse.CategoryMarginDTO> marginsByCategory = categoryOrdersMap.entrySet().stream()
            .map(entry -> {
                ...
                BigDecimal catVenda = list.stream().map(Order::getValorVenda).reduce(BigDecimal.ZERO, BigDecimal::add);
                BigDecimal catBruto = list.stream().map(Order::getLucroBruto).reduce(BigDecimal.ZERO, BigDecimal::add);
                BigDecimal catLiquido = list.stream().map(Order::getLucroLiquido).reduce(BigDecimal.ZERO, BigDecimal::add);

                BigDecimal mBruta = catVenda.compareTo(BigDecimal.ZERO) > 0 
                    ? catBruto.divide(catVenda, 4, RoundingMode.HALF_UP).multiply(BigDecimal.valueOf(100)).setScale(2, RoundingMode.HALF_UP) 
                    : BigDecimal.ZERO;

                BigDecimal mLiquida = catVenda.compareTo(BigDecimal.ZERO) > 0 
                    ? catLiquido.divide(catVenda, 4, RoundingMode.HALF_UP).multiply(BigDecimal.valueOf(100)).setScale(2, RoundingMode.HALF_UP) 
                    : BigDecimal.ZERO;

                return new DashboardResponse.CategoryMarginDTO(c.getNome(), mBruta, mLiquida, catVenda);
            })
            .collect(Collectors.toList());

        // 4. Marketplace Stats
        Map<Marketplace, List<Order>> mktOrdersMap = orders.stream().collect(Collectors.groupingBy(Order::getMarketplace));
        List<DashboardResponse.MarketplaceStatDTO> mktStats = mktOrdersMap.entrySet().stream()
            .map(entry -> {
                Marketplace m = entry.getKey();
                List<Order> list = entry.getValue();
                BigDecimal venda = list.stream().map(Order::getValorVenda).reduce(BigDecimal.ZERO, BigDecimal::add);
                BigDecimal bruto = list.stream().map(Order::getLucroBruto).reduce(BigDecimal.ZERO, BigDecimal::add);
                BigDecimal liquido = list.stream().map(Order::getLucroLiquido).reduce(BigDecimal.ZERO, BigDecimal::add);
                long pedidos = list.size();

                BigDecimal mBruta = venda.compareTo(BigDecimal.ZERO) > 0 
                    ? bruto.divide(venda, 4, RoundingMode.HALF_UP).multiply(BigDecimal.valueOf(100)).setScale(2, RoundingMode.HALF_UP) 
                    : BigDecimal.ZERO;

                BigDecimal mLiquida = venda.compareTo(BigDecimal.ZERO) > 0 
                    ? liquido.divide(venda, 4, RoundingMode.HALF_UP).multiply(BigDecimal.valueOf(100)).setScale(2, RoundingMode.HALF_UP) 
                    : BigDecimal.ZERO;

                return new DashboardResponse.MarketplaceStatDTO(m.getNome(), venda, bruto, liquido, pedidos, mBruta, mLiquida);
            })
            .collect(Collectors.toList());

        // 5. Monthly Evolution (group by Year-Month)
        ...
        Map<String, List<Order>> monthlyOrdersMap = orders.stream()
            .collect(Collectors.groupingBy(o -> o.getDataPedido().format(DateTimeFormatter.ofPattern("yyyy-MM"))));

        List<DashboardResponse.MonthlyEvolutionDTO> monthlyEvolution = monthlyOrdersMap.entrySet().stream()
            .map(entry -> {
                String yyyyMM = entry.getKey();
                List<Order> list = entry.getValue();
                BigDecimal v = list.stream().map(Order::getValorVenda).reduce(BigDecimal.ZERO, BigDecimal::add);
                BigDecimal b = list.stream().map(Order::getLucroBruto).reduce(BigDecimal.ZERO, BigDecimal::add);
                BigDecimal liq = list.stream().map(Order::getLucroLiquido).reduce(BigDecimal.ZERO, BigDecimal::add);
                long pCount = list.size();

                // Format month label nice
                String[] split = yyyyMM.split("-");
                int year = Integer.parseInt(split[0]);
                int month = Integer.parseInt(split[1]);
                Calendar cal = new GregorianCalendar(year, month - 1, 1);
                String label = cal.getDisplayName(Calendar.MONTH, Calendar.SHORT, new Locale("pt", "BR")) + " " + year;
            ...
            .sorted(Comparator.comparing(DashboardResponse.MonthlyEvolutionDTO::getMes)) // Sort alphabetically/chronologically
            .collect(Collectors.toList());
