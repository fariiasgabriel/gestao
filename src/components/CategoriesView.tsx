...
export default function CategoriesView() {
  ...
  const fetchCategories = async () => {
    ...
    try {
      ...
      const sorted = res.data.sort((a: Category, b: Category) => a.nome.localeCompare(b.nome));
      setCategories(sorted);
      setCurrentPage(1);
    } catch (err: any) {
      setError("Erro ao carregar lista de categorias.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingCategory) {
        // Edit
        await api.put(`/categories/${editingCategory.id}`, { nome });
      } else {
        // Add
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      if (err.response && err.response.data && err.response.data.message) {
        ...
        setDeleteError("Não é possível excluir esta categoria pois ela pode estar vinculada a produtos.");
      }
    }
  };
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
        {loading ? (
          <div className="p-12 text-center flex flex-col items-center gap-3">
            ...
            <span className="text-slate-500 text-sm font-medium">Buscando categorias...</span>
          </div>
        ) : (
          <>
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse text-sm">
                <thead className="bg-slate-50 text-slate-500 uppercase font-bold border-b border-slate-100">
                  <tr>
                    <th className="p-4 pl-6">Nome da Categoria</th>
            ...
            {totalPages > 1 && (
              <div className="p-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
                <span className="text-xs text-slate-500 font-mono">
                  Página {currentPage} de {totalPages} ({categories.length} registros)
                </span>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Ex: Eletrodomésticos, Moda"
